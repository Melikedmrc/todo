    ...
    <View className="flex-1 h-auto">
      <View className="flex w-96 p-4 bg-slate-50 rounded-lg">
          ...
          {['Daily', 'Weekly', 'Monthly'].map((cycle) => (
            <TouchableOpacity
              ...
              className={`px-4 py-2 rounded-full mb-2 ${
                selectedCycle === cycle ? 'bg-orange-300' : 'bg-gray-300'
              }`}
        ...
        <View className="flex-row justify-around mb-4 border-b-2 border-slate-400">
          {daysOfWeek.map((day) => (
            <TouchableOpacity
              key={day}
              className={`w-9 h-9 items-center justify-center p-1 rounded-3xl mb-2 ${
                selectedDays.includes(day) ? 'bg-orange-300' : 'bg-gray-300'
              }`}
      ...
      <View className="flex w-96 p-4 bg-slate-50 mt-5 rounded-lg">
        ...
        <View className="flex-row">
          <TouchableOpacity className="bg-blue-300 p-2 mr-2 rounded-xl">
            <Text>Daily Routine +</Text>
          </TouchableOpacity>
          <TouchableOpacity className="bg-blue-300 p-2 rounded-xl">
